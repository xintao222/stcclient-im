...
public class Decrypt {
	...
	public String encode(String in, int len) {
		...
		for (z = 0; z < x; z++) {
			...
			out[i + 1] = encodeTable((char) ((in.charAt(j) & 0x03) << 4 | in
					.charAt(j + 1) >> 4));
			out[i + 2] = encodeTable((char) ((in.charAt(j + 1) & 0x0F) << 2 | in
					.charAt(j + 2) >> 6));
	...
	public static final byte[] str2Bcd(String str) {
		...
		int i = 0;
		while (i < str.length()) {
			bb.put((byte) ((Integer.parseInt(sb.substring(i, i + 1))) << 4 | Integer
	...
	public String getSmsInfos(String encoded) {
		try {
			...
			/* ���ݱ���2.0 */
				...
				// MessageNum
				pos += 4;
				String MessageNum = String.valueOf(new Integer(a[pos]));
				// MessageIndex
				String MessageIndex = String.valueOf(new Integer(a[pos + 1]));
				// Reserve
				pos += 2;
				String Reserve = String.valueOf(new Integer(a[pos]));
				// AppType
				String AppType = String.valueOf(new Integer(a[pos + 1]));
				pos += 2;
				// userLayer
				userLayer = new String(a, pos, a.length - pos);
				return "2/" + hsman + "/" + hstype + "/" + IMEI + "/" + IMSI
						+ "/" + appid + "/" + vmver + "/" + hsver + "/"
						+ MessageNum + "/" + MessageIndex + "/" + Reserve + "/"
						+ AppType + "/" + userLayer;
			}
			/* ���ݱ���2.1 */
			else if ((a[0] == 0x10)) {
				...
				// MessageIndex
				String MessageIndex = new String().valueOf(new Integer(
						a[pos + 1]));
				// Reserve
				// pos += 2;
				...
				return "3/" + appid + "/" + vmver + "/" + hsver + "/"
						+ manufactory + "/" + HandsetType + "/" + IMSI + "/"
						+ MessageNum + "/" + MessageIndex + "/" + userLayer;// +
																			// "/"
																			// +
																			// Reserve;
			}
