...
import stc.foundation.codec.DecodeContextFactory;
import stc.foundation.codec.bean.tlv.annotation.TLVAttribute;
import stc.foundation.codec.bean.tlv.decode.TLVDecodeContext;
import stc.foundation.codec.bean.tlv.decode.TLVDecodeContextFactory;
import stc.foundation.codec.bean.tlv.decode.TLVDecoder;
import stc.foundation.codec.bean.tlv.decode.TLVDecoderOfBean;
...
public class BeanTLVDecoder implements TLVDecoderOfBean {
	 ...
	 * 
	 * @see
	 * com.skymobi.bean.tlv.decode.decoders.TLVDecoderOfBean#getDecodeContextFactory
	 * ()
	 */
	...
	// public void setDecodeContextFactory(TLVDecodeContextFactory
	// decodeContextFactory) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.skymobi.bean.tlv.decode.decoders.TLVDecoderOfBean#decode(int,
	 * byte[], com.skymobi.bean.tlv.decode.TLVDecodeContext)
	 */
	...
	public Object decode(int tlvLength, byte[] tlvValue, TLVDecodeContext ctx) {
		...
		if (null == target) {
			throw new RuntimeException(
					"BeanTLVDecoder, can not create instance.");
		}
		...
		while (offset + 8 <= tlvLength) {
			...
			byte[] valueBytes = ArrayUtils.subarray(tlvValue, offset, offset
					+ len);
			offset += len;
			...
			try {
				bean = decoder.decode(
						len,
						valueBytes,
						getDecodeContextFactory().createDecodeContext(type,
								field));
