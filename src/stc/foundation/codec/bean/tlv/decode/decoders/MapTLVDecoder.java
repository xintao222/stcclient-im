...
public class MapTLVDecoder implements TLVDecoder {
	...
	public Object decode(int tlvLength, byte[] tlvValue, TLVDecodeContext ctx) {
		...
		TLVDecoder keyDecoder = ctx.getDecoderRepository().getDecoderOf(
				(Class<?>) keyType);
		TLVDecoder valueDecoder = ctx.getDecoderRepository().getDecoderOf(
				(Class<?>) valueType);

		if (null == keyDecoder || null == valueDecoder) {
			throw new RuntimeException("unknown decoder for [type]: " + keyType
					+ " and " + valueType);
		}
		...
		while (offset + 4 <= tlvLength) {
			...
			byte[] objBytes = ArrayUtils.subarray(tlvValue, offset, offset
					+ len);
			offset += len;
			try {
				if (isDecodeKey) {
