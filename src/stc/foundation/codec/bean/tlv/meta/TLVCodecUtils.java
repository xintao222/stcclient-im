...
import stc.foundation.codec.bean.util.meta.DefaultInt2TypeMetainfo;
...
public class TLVCodecUtils {
	...
	static public Field[] getTLVFieldsOf(final Class<?> tlvType) {
		return tlvFieldsCache.get(tlvType, new Callable<Field[]>() {
			...
			public Field[] call() throws Exception {
				return FieldUtils.getAnnotationFieldsOf(tlvType,
						TLVAttribute.class);
			}
		});
	}
	...
	static public Int2TypeMetainfo createTopmostTypeMetainfo(
			Collection<String> packages) {
		return BeanMetainfoUtils.createTypeMetainfo(packages, CLS2INT);
	}

	static public Int2TypeMetainfo createTopmostTypeMetainfoByClasses(
